"""Train EfficientNetV2-S+CBAM as nevus-vs-melanoma. Does not overwrite the 3-class bundle.

Outputs under models/binary/ and artifacts/binary/.
"""
...
OUT_MODEL = OUT_DIR / "cnn_v2s_binary_lesion.pth"
CLASSES = {"nevus", "melanoma"}
EPOCHS = 12


def collect_binary(split: str) -> list[Path]:
    root = paths.LESION / split
    paths_ = []
    for cls in ("nevus", "melanoma"):
        folder = root / cls
        if folder.exists():
            paths_.extend(sorted(folder.glob("*.png")))
    return paths_
...
def main() -> None:
    ...
    print(f"device={cnn.device}  train={len(train_paths)}  val={len(val_paths)}")
    ...
    print(f"wrote {OUT_MODEL}  best_val_acc={best_acc:.4f}")
    (OUT_DIR / "cnn_v2s_binary_lesion_log.txt").write_text(
